...
# srun -p ai4earth --quotatype=spot --ntasks-per-node=$single_gpus --cpus-per-task=$cpus -N $node_num -o job/%j.out --gres=gpu:$single_gpus --async -u python scripts/run_assimilation_loop.py --config $config --prefix $prefix --date $date
